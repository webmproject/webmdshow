...
#include "vpxdecoderfilter.hpp"
#include "vpxdecoderoutpin.hpp"
...
HRESULT Inpin::GetName(PIN_INFO& info) const {
  ...
  if (m_connection_mtv.Empty() == false) {
    if (m_connection_mtv[0].formattype == WebmTypes::MEDIASUBTYPE_VP80)
      name = kVP8PinName;
    else  // (m_connection_mtv[0].formattype == WebmTypes::MEDIASUBTYPE_VP90)
      name = kVP9PinName;
  }
...
HRESULT Inpin::Start() {
  ...
  } else if (m_connection_mtv[0].formattype == WebmTypes::MEDIASUBTYPE_VP90) {

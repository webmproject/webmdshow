...
function install_xiph_files() {
  local project="$1"
  if [[ -z "$project" ]]; then
    die "${FUNCNAME[0]}: no project specified [arg1 empty]"
  fi
  local src_dir="$2"
  if [[ -z "$src_dir" ]]; then
    die "${FUNCNAME[0]}: no src_dir specified [arg2 empty]"
  fi
  local target_dir="$3"
  if [[ -z "$target_dir" ]]; then
    die "${FUNCNAME[0]}: no target_dir specified [arg3 empty]"
  fi
  local inc_dir="$4"
  if [[ -z "$inc_dir" ]]; then
    die "${FUNCNAME[0]}: no inc_dir specified [arg4 empty]"
  fi
...
function clone_libvpx_and_checkout_tag() {
  ...
  local clone_dir="$1"
  if [[ -z "$clone_dir" ]]; then
    die "${FUNCNAME[0]}: no clone_dir specified [arg1 empty]"
  fi
  local git_remote="$2"
  if [[ -z "$git_remote" ]]; then
    die "${FUNCNAME[0]}: no git_remote specified [arg2 empty]"
  fi
  local git_checkout_target="$3"
  if [[ -z "$git_checkout_target" ]]; then
    die "${FUNCNAME[0]}: no git_checkout_target specified [arg3 empty]"
  fi
  # Just clone into .; the libvpx directory will be deleted later. 
  local tmp_path=.
  if [[ ! -d "${tmp_path}" ]]; then
    die "${FUNCNAME[0]}: temp dir creation failed [tmp_path does not exist]"
  fi
  ...
  if [[ ! -d "${tmp_path}/${clone_dir}" ]]; then
    die "${FUNCNAME[0]}: clone failed [directory does not exist]"
  fi
...
function build_libvpx() {
  local clone_dir="$1"
  if [[ -z "$clone_dir" ]]; then
    die "${FUNCNAME[0]}: clone_dir not specified [arg1 empty]"
  fi
...
function install_libvpx_files() {
  ...
  for (( target_num=0; target_num < "${#targets_array[@]}"; target_num++ )); do
    ...
    if [[ "${lib_src_subdir}" == "x86-" ]]; then
      ...
      die "${FUNCNAME[0]}: unexpected entry in target array substr\

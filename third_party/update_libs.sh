...
function download_and_untar() {
  ...
  if [[ -z "$url" ]]; then
    die "download_and_untar: no url specified [arg1 empty]"
  fi
  ...
  if [[ -n $(which wget) ]]; then
    ...
    die "download_and_untar: either wget or curl must be in \$PATH"
  fi
  if [[ ! -e "${outfile}" ]]; then
    die "download_and_untar: download failed [argurl]"
  fi
...
"${src_dir}/win32/VS2008/${devenv_outdir_array[$lib_index]}/vc90.pdb"
    if [[ -e "${pdb_path_ogg}" ]]; then
      ...
      local readonly pdb_path_vorbis=\

...
# Installs libyuv includes and libraries. Includes come from the git repo
# in $WORK_DIR/$LIBYUV. Libraries are picked up from the output locations of
# the projects built by build_libs().
install_libyuv() {
  ...
  cp -rp "${THIRD_PARTY}/${WORK_DIR}/${LIBYUV}/include/" "${target_dir}/include"
  vlog "Done."

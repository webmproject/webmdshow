...
HRESULT Outpin::QueryAccept(const AM_MEDIA_TYPE* pmt_query)
{
    ...
    if (mt_query.formattype == FORMAT_VideoInfo)
        return QueryAccept_VideoInfo(mt_in, mt_query);

    if (mt_query.formattype == FORMAT_VideoInfo2)
        return QueryAccept_VideoInfo2(mt_in, mt_query);

    return S_OK;
}


HRESULT Outpin::QueryAccept_VideoInfo(
  ...
  const AM_MEDIA_TYPE& mt_out)
{
    ...
    const VIDEOINFOHEADER& vih_out = (VIDEOINFOHEADER&)(*mt_out.pbFormat);
...
HRESULT Outpin::QueryAccept_VideoInfo2(
  const AM_MEDIA_TYPE& mt_in,
  const AM_MEDIA_TYPE& mt_out)
{
    assert(mt_in.formattype == FORMAT_VideoInfo);
    ...
    const VIDEOINFOHEADER2& vih2_out = (VIDEOINFOHEADER2&)(*mt_out.pbFormat);
    ...
    if (!(vih2_out.dwInterlaceFlags & AMINTERLACE_IsInterlaced))
        __noop;  //TODO: vet flags, when IsInterlaced is true
...
void Outpin::OnInpinConnect(const AM_MEDIA_TYPE& mtIn)
{
    ...
    AddPreferred(MEDIASUBTYPE_NV12,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);

    AddPreferred(MEDIASUBTYPE_YV12,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);

    AddPreferred(WebmTypes::MEDIASUBTYPE_I420,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);
    ...
    AddPreferred(MEDIASUBTYPE_UYVY,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);

    AddPreferred(MEDIASUBTYPE_YUY2,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);

    AddPreferred(MEDIASUBTYPE_YUYV,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);

    AddPreferred(MEDIASUBTYPE_YVYU,
                 vihIn.AvgTimePerFrame,
                 w, h,
                 dwBitCount, dwSizeImage);
}
...
void Outpin::AddPreferred(const GUID& subtype,
                          ...
                          DWORD dwSizeImage)
{
    AddVIH2(m_preferred_mtv, subtype, AvgTimePerFrame, width, height, dwBitCount, dwSizeImage);
    AddVIH(m_preferred_mtv, subtype, AvgTimePerFrame, width, height, dwBitCount, dwSizeImage);
}
    ...
    DWORD dwSizeImage)
{
    ...
    //SetRectEmpty(&vih2.rcSource);  //TODO
    //SetRectEmpty(&vih2.rcTarget);  //TODO
    vih2.dwBitRate = 0;

...
HRESULT Inpin::Receive(IMediaSample* pInSample)
{
    ...
    if (m_pFilter->m_decimate > 1)
    {
        ...
        {
            if (m_frames_received == 1 || num_since_drop == 1)
            {
                // Don't assume a constant input frame rate or 0 start time.
                // Update |m_decimate_start_time| on the first frame dropped of
                // each sequence of discarded frames.

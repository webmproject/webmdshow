    ...
    const AM_MEDIA_TYPE* pmt)
{
    ...
    {
        hr = QueryAccept(pmt);
        //TODO: accept partial media type?
    ...
    assert(mt.formattype == VorbisTypes::FORMAT_Vorbis2);
    assert(mt.cbFormat >= sizeof(FMT));
    assert(mt.pbFormat);

    const FMT& fmt = (FMT&)(*mt.pbFormat);

    const long bytes_per_sample = 2;  //assume 16 bits
    const long block_align = fmt.channels * bytes_per_sample;
    const long avg_bytes_per_sec = block_align * fmt.samplesPerSec;
    const long cbBuffer = avg_bytes_per_sec / 2;  //TODO: estimate better
...
HRESULT Outpin::QueryAccept(const AM_MEDIA_TYPE* pmt)
{
    ...
    if (mtOut.subtype != VorbisTypes::MEDIASUBTYPE_Vorbis2)
        return S_FALSE;

    if (mtOut.formattype != VorbisTypes::FORMAT_Vorbis2)
        return S_FALSE;
    ...
    if (mtOut.cbFormat < sizeof(FMT))
        return S_FALSE;

    const FMT& fmtOut = (FMT&)(*mtOut.pbFormat);

    const AM_MEDIA_TYPE& mtIn = inpin.m_connection_mtv[0];
    const WAVEFORMATEX& wfxIn = (WAVEFORMATEX&)(*mtIn.pbFormat);

    if (fmtOut.channels != wfxIn.nChannels)
        return S_FALSE;

    if (fmtOut.samplesPerSec != wfxIn.nSamplesPerSec)
        return S_FALSE;

    //TODO: check bits/sample
    //TODO: what about vorbis headers (ident, setup, comment)?
...
{
    const wchar_t* const name_ = L"Vorbis";
...
HRESULT Outpin::Seize(CLockable::Lock& lock)
{
    return lock.Seize(m_pFilter);
}

FILTER_STATE Outpin::GetState()
{
    return m_pFilter->m_state;
}

IBaseFilter* Outpin::GetFilter()  //no addref here
{
...
void Outpin::OnInpinConnect(const AM_MEDIA_TYPE& mtIn)
{
    ...
    m_preferred_mtv.Add(mtIn);
}


void Outpin::OnInpinDisconnect()
{
    if (bool(m_pPinConnection))
    {
...
void Outpin::SetDefaultMediaTypes()
{
    ...
    //Create a partial media type; this is the best we can do
    //without having an inpin connection (hence no media type).
...
#ifdef _DEBUG
    wodbgstream os;
    os << "webmvorbisenc::Outpin[" << m_id << "]::StartThread: hThread=0x"
       << hex << h << dec
       << endl;
#endif
}
...
#ifdef _DEBUG
    wodbgstream os;
    os << "webmvorbisenc::Outpin[" << m_id << "]::StopThread: hThread=0x"
...
#ifdef _DEBUG
            odbgstream os;
            os << "webmvorbisenc::outpin::EOS: calling pin->EOS" << endl;
#endif
...
#ifdef _DEBUG
            os << "webmvorbisenc::outpin::EOS: called pin->EOS; hr=0x"
               << hex << hrEOS << dec
               << endl;
#endif
        }
